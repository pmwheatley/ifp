...
DIRECTORY = hugo_build
HEADER = hugo
DSO = hugo-3.1.so
ARCHIVE = $(INTERPRETERS)/hugov31_source.tar.gz
ARCHIVE_2 = $(INTERPRETERS)/hugov31_glk_source.zip
...
$(DIRECTORY): $(ARCHIVE)
	...
	cd $(DIRECTORY)/source; \
		unzip ../../$(ARCHIVE_2); \
		mv heglk.c heglk.c.old; mv heglk.c.graphics heglk.c
	cd $(DIRECTORY); \
		patch -i ../$(PATCHES)/hugo_cleanup.patch -Np1; \
		patch -i ../$(PATCHES)/hugo.patch -Np1

$(PLUGINS)/$(DSO): $(HEADER)_plugin.o $(DIRECTORY)
	mkdir -p $(PLUGINS)
	cd $(DIRECTORY)/source; \
		$(MAKE) -f Makefile.unix \
		OFLAGS="-O2 -fPIC -D__NO_STRING_INLINES" $(DSO)
	cp $(DIRECTORY)/source/$(DSO) $(PLUGINS)
	strip -g $(PLUGINS)/$(DSO)
