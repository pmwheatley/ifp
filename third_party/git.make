...
HEADER = git
DSO = git-1.1.3.so
ARCHIVE = $(INTERPRETERS)/git-113.zip
...
$(DIRECTORY): $(ARCHIVE)
	...
	cd $(DIRECTORY); \
		unzip ../$(ARCHIVE); \
		patch -i ../$(PATCHES)/git_unicode.patch -Np1; \
		patch -i ../$(PATCHES)/git.patch -Np1

$(PLUGINS)/$(DSO): $(HEADER)_plugin.o $(DIRECTORY)
	mkdir -p $(PLUGINS)
	cd $(DIRECTORY)/git-1.1.3; \
		$(MAKE) CC="$(CC) -D__NO_STRING_INLINES" $(DSO)
	cp $(DIRECTORY)/git-1.1.3/$(DSO) $(PLUGINS)
	strip -g $(PLUGINS)/$(DSO)
